...
`--resolve` hands the run the neighboring documents so those references can
be settled. It is repeatable and takes files or directories:

```
ctdl-validate credential.json --resolve owner.json --resolve neighbors/
```
...
else — it lets a staged document resolve references against neighbors you
already hold, and the neighbors are never themselves validated.
...
the page's own `<meta charset>`, then a labeled `errors="replace"` fallback --
