...
``fail-on`` is the one thing the CLI has no flag for. ``ctdl-validate`` gates
...
the tool counts it as neither a pass nor a fail.
...
:func:`ctdl_validate.sarif.merge_logs`, imported rather than reimplemented,
...
def run_cli(
    ...
    # The action's inputs reach this call, and that is the point: they are
    # argv entries in a list, with no shell to interpret them, so a caller
    # controls what is validated and never what is executed.
...
def report_findings(document: Path, findings: list[dict[str, str]]) -> None:
    for finding in findings:
        ...
        where = f"{finding['entity']}: {finding['property']} = {finding['value']}"
        annotate(
...
def describe_unreadable(report: object) -> str | None:
    ...
    passed. That is an absence published as a measurement, which is the defect
    this tool exists to report. A count that is not there is not a count of
    none, so a report missing any part of the contract fails the run (exit 2)
...
def validate_one(
    ...
    print(f"{document}: " + ", ".join(f"{summary[s]} {s}" for s in SEVERITIES))
    return True
...
def write_sarif(destination: Path, logs: list[object]) -> bool:
    ...
    from ctdl_validate.sarif import merge_logs  # noqa: PLC0415
...
def main() -> int:
    fail_on = (os.environ.get("CTDL_FAIL_ON") or "error").strip().lower()
    if fail_on not in GATED:
        annotate("error", f"fail-on must be one of {', '.join(GATED)}, not {fail_on!r}")
        return EXIT_USAGE

    raw = (os.environ.get("CTDL_PATH") or "").strip()
    if not raw:
    ...
    resolve = (os.environ.get("CTDL_RESOLVE") or "").split()
    sarif_file = (os.environ.get("CTDL_SARIF_FILE") or "").strip()
