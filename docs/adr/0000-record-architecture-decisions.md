...
ctdl-validate makes a small number of consequential, hard-to-reverse
...
code), why the schema snapshots are vendored, and where the version floors
sit. The reasoning behind a structural choice must not live only in a commit
message, or a later change will either re-litigate a settled question or
