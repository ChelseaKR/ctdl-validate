...
def _sarif_run(tmp_path: Path, path: str, **inputs: str) -> tuple[int, str, Path]:
    destination = tmp_path / "out" / "ctdl-validate.sarif"
    code, stdout, _ = _run(tmp_path, CTDL_PATH=path, CTDL_SARIF_FILE=str(destination), **inputs)
    return code, stdout, destination


def test_no_sarif_is_written_unless_it_is_asked_for(tmp_path: Path) -> None:
    code, _, _ = _run(tmp_path, CTDL_PATH=str(FIXTURES / "clean_framework.json"))
    assert code == 0
...
def test_the_sarif_file_carries_every_document_as_one_run(tmp_path: Path) -> None:
    ...
    code, stdout, destination = _sarif_run(tmp_path, str(documents), CTDL_FAIL_ON="error")
    assert code == 1, stdout  # both fixtures carry an ERROR finding
    ...
    assert [d["path"] for d in run["properties"]["documents"]] == [
        (documents / name).as_uri() for name in sorted(names)
    ]
    assert run["properties"]["summary"]["ERROR"] == 3
...
def test_the_sarif_file_records_which_vendored_snapshot_decided_it(tmp_path: Path) -> None:
    code, stdout, destination = _sarif_run(tmp_path, str(FIXTURES / "clean_framework.json"))
    assert code == 0, stdout
    log = json.loads(destination.read_text(encoding="utf-8"))
    recorded = log["runs"][0]["tool"]["driver"]["properties"]["vendoredSnapshot"]
    assert recorded["algorithm"] == "sha256"
    assert len(recorded["files"]) == 4
...
def test_no_sarif_is_written_when_a_document_could_not_be_read(tmp_path: Path) -> None:
    ...
    documents = tmp_path / "documents"
    documents.mkdir()
    (documents / "clean.json").write_bytes((ROOT / FIXTURES / "clean_framework.json").read_bytes())
...
def _sarif_stub(tmp_path: Path, report: object, sarif: object) -> Path:
    ...
    ``ctdl_validate.sarif`` -- which the runner imports to merge -- is still
    ...
    (package / "__init__.py").write_text(
        f"__path__.append({str(ROOT / 'src' / 'ctdl_validate')!r})\n", encoding="utf-8"
    )
    (package / "__main__.py").write_text(
        "import json, sys\n"
...
def _run_with_sarif_stub(tmp_path: Path, report: object, sarif: object) -> tuple[int, str, Path]:
    ...
    destination = tmp_path / "ctdl-validate.sarif"
    completed = subprocess.run(
        [sys.executable, str(RUNNER)],
        capture_output=True,
        text=True,
        ...
        env={
            **os.environ,
            ...
            "CTDL_PATH": str(FIXTURES / "clean_framework.json"),
            "CTDL_SARIF_FILE": str(destination),
        },
        check=False,
    )
...
def _whole_sarif(results: int) -> dict[str, object]:
    return {
                        ...
                        "rules": [
                            {
                                ...
                                "properties": {
                                    "sources": [
                                        {"url": "https://credreg.net/x", "retrieved": "2026-01-01"}
                                    ]
                                },
                            }
                        ],
...
def _report_with(findings: int) -> dict[str, object]:
    report = _whole_report()
    report["findings"] = [
        {
            "code": "X",
...
def test_the_action_inputs_and_the_runner_read_the_same_environment() -> None:
    ...
    notices."""
    source = RUNNER.read_text(encoding="utf-8")
    read = set(re.findall(r'os\.environ\.get\("(CTDL_[A-Z_]+)"\)', source))
    passed = set(re.findall(r"^\s+(CTDL_[A-Z_]+):", (ROOT / "action.yml").read_text("utf-8"), re.M))
...
# severity is a minor bump, and the runner accepts later minors of the same
# major on purpose, so both were reachable without any change here.


def _runner_module() -> Any:
    """The runner imported as a module, for the two constants a test reads directly.

    ``importlib`` with a literal name rather than an ``import`` statement inside
    a function: the statement trips PLC0415, and interpolating a name into
    ``import_module`` is the thing ``cli.py`` declines to do for its own reasons.
    """
    sys.path.insert(0, str(ROOT / "tools"))
    try:
        return importlib.import_module("action_runner")
    finally:
        sys.path.pop(0)


def _report_with_severity(name: str) -> dict[str, object]:
    report = _whole_report()
    report["findings"] = [
        {
            "code": "SOMETHING_NEW",
            "severity": name,
            "entity": "urn:example:1",
...
def test_a_finding_of_every_known_severity_is_still_read(name: str, tmp_path: Path) -> None:
    ...
    assert name in runner.SEVERITIES, "this list and the runner's have drifted"
